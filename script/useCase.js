$(function() {
	if($('#noResult').text() != 'Non ci sono Use Case.') {
...
function filterTable() {
	...
	$('#allUC tbody tr').each(function() {
		...
		if($(this).children().eq(colNome).text().toUpperCase().indexOf($('#name_UC').val().toUpperCase()) > -1 || $(this).children().eq(colCodice).text().toUpperCase().indexOf($('#name_UC').val().toUpperCase()) > -1)
			$(this).show();
		else
			$(this).hide();
	});
